...
void enqueue(const Lagu &lagu)
{
    if (isFull())
    {
        cout << "Playlist penuh, lagu tidak dapat ditambahkan." << endl;
    }
    else
    {
        if (isEmpty())
...
void dequeue()
{
    if (isEmpty())
    {
        cout << "Playlist kosong, tidak ada lagu yang dihapus." << endl;
    }
    else
    {
        cout << "Menambah lagu: " << queue.isi[queue.front].judul << ", "
             ...
             << queue.isi[queue.front].rating << endl;
...
void tambahLaguKeCsv(const string &username, const string &playlist)
{
    string filename = "dataLaguUser/" + username + "/" + playlist + ".csv";
    ...
    if (!fs::exists("dataLaguUser/" + username))
    {
        fs::create_directory("dataLaguUser/" + username);
    }
    ...
    while (!isEmpty())
    {
        ...
        {
            cout << "Lagu dengan judul \"" << judulLagu << "\" sudah ada dalam playlist." << endl;
        }
        else
        {
...
bool displayQueue(const string &username, const string &playlist)
{
    string filename = "dataLaguUser/" + username + "/" + playlist + ".csv";
    ifstream file(filename);
    ...
    int index = 1;
    bool isEmpty = true;
    cout << "Isi Playlist " << playlist << ":" << endl;
    while (getline(file, line))
    {
        isEmpty = false;
        stringstream ss(line);
        string judul, artis, genre, rating;
        getline(ss, judul, ',');
        getline(ss, artis, ',');
        getline(ss, genre, ',');
        getline(ss, rating, ',');
        cout << "[" << index++ << "] " << judul << " - " << artis << " - " << genre << " - " << rating << endl;
    }
...
void deleteQueue(const string &judul, const string &username, const string &playlist)
{
    string filename = "dataLaguUser/" + username + "/" + playlist + ".csv";
    ifstream file(filename);
    if (!file.is_open())
    ...
    {
        stringstream ss(line);
        string judulLagu, artis, genre, rating;
        getline(ss, judulLagu, ',');
        getline(ss, artis, ',');
        getline(ss, genre, ',');
        getline(ss, rating, ',');
        ...
        {
            found = true;
            cout << "Lagu " << judul << " sudah keluar dari playlist " << playlist << ".\n"
                 << endl;
        }
        else
        {
            laguList.push_back({judulLagu, artis, genre, rating});
        }
    }
    file.close();
    ...
    if (!found)
    {
        cout << "Judul lagu tidak ditemukan dalam playlist.\n"
             << endl;
        return;
    }
...
bool findQueue(const string &target, const string &username, const string &playlist)
{
    string filename = "dataLaguUser/" + username + "/" + playlist + ".csv";
    ifstream file(filename);
    if (!file.is_open())
...
Lagu getLaguInfo(const string &judul, const string &filename)
{
    ...
    while (getline(file, line))
    {
        ...
        {
            file.close();
            return {judulLagu, artis, genre, rating};
        }
    }
...
void dfs(int start, vector<Lagu> &rekomendasi, const vector<string> &artisQueue, const vector<string> &genreQueue, const string &username, const string &playlist)
{
    visited[start] = true;
    for (int neighbor : adj[start])
    {
        if (!visited[neighbor])
        {
            if (!findQueue(daftarLagu[neighbor].judul, username, playlist) &&
            ...
            dfs(neighbor, rekomendasi, artisQueue, genreQueue, username, playlist);
        }
    }
}
...
void buatGraphCsv()
{
    ...
    while (getline(file, line))
    {
        ...
        daftarLagu.push_back({judul, artis, genre, rating});
    }
...
void ubahRatingLagu(const string &username, const string &playlist)
{
    string filename = "dataLaguUser/" + username + "/" + playlist + ".csv";
    string filename2 = "dataLagu.csv";
    ifstream file(filename);
    ...
    if (!file2.is_open())
    {
        cout << "Tidak dapat membuka file " << filename2 << endl;
        return;
    }

    vector<Lagu> laguList;
    vector<Lagu> laguList2;
    string line;
    string line2;
    bool found = false;
    bool found2 = false;

    // Baca konten file ke vector
    while (getline(file, line))
    {
        stringstream ss(line);
        string judul, artis, genre, rating;
        ...
        laguList.push_back({judul, artis, genre, rating});
    }
    file.close();
    ...
    while (getline(file2, line2))
    {
        ...
        laguList2.push_back({judul, artis, genre, rating});
    }
    file2.close();

    // Input judul lagu yang ingin diubah ratingnya
    string judulLagu;
    cout << "+-----------------------------+\n";
    ...
    string ratingBaru;
    cout << "+-----------------------------+\n";
    cout << "| " << left << setw(27) << ("Masukkan Rating Baru") << " |\n";
    cout << "+-----------------------------+\n";
    cout << "| ";
    getline(cin, ratingBaru);
    cout << "+-----------------------------+\n";

    // Cari lagu berdasarkan judul dan ubah ratingnya
    for (auto &lagu : laguList)
    {
        if (lagu.judul == judulLagu)
        {
            lagu.rating = ratingBaru;
            found = true;
            break;
        }
    }
    ...
    if (!found)
    {
        cout << "Judul lagu tidak ditemukan dalam playlist." << endl;
        return;
    }

    for (auto &lagu2 : laguList2)
    {
        if (lagu2.judul == judulLagu)
        {
            lagu2.rating = ratingBaru;
            found2 = true;
            break;
        }
    }
    ...
    // Tulis kembali konten yang diperbarui ke file
    ofstream outFile(filename);
    if (!outFile.is_open())
    {
        cout << "Tidak dapat membuka file untuk menulis data." << endl;
        return;
    }
    ...
    for (const auto &lagu2 : laguList2)
    {
        outFile2 << lagu2.judul << ","
                 << lagu2.artis << ","
                 << lagu2.genre << ","
                 << lagu2.rating << "\n";
    }
    ...
    outFile2.close();
    cout << "Rating lagu berhasil diubah." << endl;
}
    ...
    do
    {
        ...
        else if (pilihan == "2")
        {
            if (loginUser(user))
            {
                ...
                string filename = "dataLaguUser/" + user.username;
                ...
                while (true)
                {
                    ...
                    if (opsiMenu == "1")
                    {
                        while (true)
                        {
                            ...
                            cout << "+-----------------------------+\n";
                            cout << "| " << left << setw(27) << ("Masukkan Nama Playlist") << " |\n";
                            cout << "+-----------------------------+\n";
                            cout << "| ";
                            string namaPlaylist;
                            ...
                            else
                            {
                                if (cekJudul(judul, DATA_LAGU)) // Ganti dengan nama file yang berisi data lagu
                                {
                                    ...
                                    enqueue(lagu);
                                    tambahLaguKeCsv(user.username, namaPlaylist);
                                }
                                else
                                {
                    ...
                    else if (opsiMenu == "2")
                    {
                        ...
                        while (true)
                        {
                            ...
                            getline(cin, namaPlaylist);
                            cout << "+-----------------------------+\n";
                            displayQueue(user.username, namaPlaylist);
                            cout << endl;

                            ifstream file(filename + "/" + namaPlaylist + ".csv");
                            if (!file.is_open())
                            {
                                system("pause");
                                break;
                            ...
                            {
                                deleteQueue(judul, user.username, namaPlaylist);
                                system("pause");
                                break;
                            }
                        }
                    ...
                    else if (opsiMenu == "3")
                    {
                        ...
                        getline(cin, namaPlaylist);
                        cout << "+-----------------------------+\n";
                        bool isDisplay = displayQueue(user.username, namaPlaylist);
                        if (isDisplay)
                        {
                            while (true)
                            {
                                cout << "\nIngin Memberi Rating Pada Lagu Atau Tidak (y/t)? ";
                                cin >> pilihanRate;
                                if (pilihanRate[0] == 'y' || pilihanRate[0] == 'Y')
                                {
                                    ubahRatingLagu(user.username, namaPlaylist);
                                    system("pause");
                                    break;
                                }
                                else if (pilihanRate[0] == 't' || pilihanRate[0] == 'T')
                                {
                                    break;
                                }
                                else
                    ...
                    else if (opsiMenu == "4")
                    {
                        ...
                        while (getline(file, line))
                        {
                            ...
                            if (songIndex != -1)
                            {
                                ...
                                dfs(songIndex, daftarRekomendasi, artisQueue, genreQueue, user.username, namaPlaylist);
                            }
                        }
                        file.close();
                        ...
                        sort(daftarRekomendasi.begin(), daftarRekomendasi.end(), [](const Lagu &a, const Lagu &b)
                             { return stoi(a.rating) > stoi(b.rating); });
                        ...
                        while (start < totalRekomendasi)
                        {
                            ...
                            {
                                cout << "[" << i + 1 << "] " << daftarRekomendasi[i].judul << " - " << daftarRekomendasi[i].artis << " - " << daftarRekomendasi[i].genre << " - " << daftarRekomendasi[i].rating << endl;
                            }
